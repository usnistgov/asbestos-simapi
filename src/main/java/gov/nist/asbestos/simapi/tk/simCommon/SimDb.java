...
//			return false
	...
	public static List<SimId> getSimIdsForUser(TestSession user) {
	...
	static  String getTransactionDirName(gov.nist.asbestos.simapi.tk.actors.TransactionType tt)  {
		return tt.getShortName();
	}

	 File getTransactionDir(gov.nist.asbestos.simapi.tk.actors.TransactionType tt) {
...
//					if (date == null) continue;  // only interested in transactions that have dates
		...
		t.nameInterpretedAsTransactionType = gov.nist.asbestos.simapi.tk.actors.TransactionType.find(t.trans);
